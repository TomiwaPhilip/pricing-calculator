...
| Field  | Description |
| ----- | ----- |
| Subtotal  | Sum of (qty × unit price) before discounts |
...
Pick a rounding policy (e.g. round to 2 decimal places per line) and apply it consistently. Document your policy in the README. 
...
Document whether users can duplicate a finalized document into a new draft (stretch goal). 
...
Validation with specific error messages (e.g. invalid percent, negative quantity). 

Sample document 

Use this to verify your calculations. Expected results below assume round to 2 decimal places per line (tax and totals).
...
2\. Tax percent is applied on the discounted line amount (Widget A: 5% of 180, not of 200). 3\. A line may have percent discount or fixed discount, not both. 

4\. Fixed discount must not exceed that line’s subtotal (reject or clamp — document your choice). 5\. If you use a different rounding policy, document it and keep results internally consistent. 
...
Tests: Unit tests on the calculation module are strongly appreciated — this is the highest-value test surface in this assignment. 

Money handling: Avoid floating-point drift. Use a strategy appropriate to your language (e.g. integer cents, decimal library). 
...
If anything is ambiguous, make a reasonable assumption, document it in your README, and proceed. Good luck — we look forward to reviewing your work.